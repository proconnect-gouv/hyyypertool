...
} from "#src/testing/response_templates";
import {
  insert_admin,
  insert_jeanbon,
  insert_moderateur,
...

{
  ...
  consola.debug(`🚮 DELETE ${deleted.length} users`);
}

{
  const deleted = await db.delete(schema.response_templates).returning();
  consola.debug(`🚮 DELETE ${deleted.length} response_templates`);
}
...
for (const insert of [insert_admin, insert_jeanbon, insert_moderateur]) {
  ...
  consola.debug(`🌱 INSERT user ${user.email} (${user.role})`);
}
...
for (const insert of [
  ...
  const user = await insert(db);
  consola.debug(`🌱 INSERT response_templates ${user.label})`);
}

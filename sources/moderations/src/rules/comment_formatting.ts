/**
 * Pure functions for formatting moderation comments.
 *
 * Migrated from moderations/lib/src/comment_message.ts
 */
...
/**
 * Formats a single comment entry based on comment type.
 *
 * @pure - No side effects, deterministic output
 */
export function format_comment_entry(comment_type: Comment_Type): string {
...
/**
 * Appends a new comment entry to existing comments.
 *
 * @pure - No side effects, deterministic output
 * @param comment - Existing comment string (can be null)
 * @param comment_type - Type of comment to append
 * @returns New comment string with entry appended
 */
export function append_comment(
  comment: string | null,
  comment_type: Comment_Type,
): string {
  return [...(comment ? [comment] : []), format_comment_entry(comment_type)].join(
    "\n",
  );
}

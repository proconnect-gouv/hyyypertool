...
// Mock Crisp API calls
const mockFetch = mock(() =>
  Promise.resolve(
    new Response(JSON.stringify({ data: { session_id: "mock-session-id" } }), {
      status: 200,
...
test("PATCH /moderations/:id/rejected - rejects moderation with reason", async () => {
  await create_unicorn_organization(pg);
  await create_adora_pony_user(pg);
  const moderation_id = await create_adora_pony_moderation(pg, {
    type: "test",
    ticket_id: "session_test_ticket_id",
  });
  ...
  // Verify database state - moderation marked as rejected
  ...
  expect(updated_moderation).toMatchInlineSnapshot(`
    {
      "comment": "7952342400000 admin@example.com | Rejeté par admin@example.com | Raison : "Invalid request"",
